function dydt = ieeeG1_SS11(t, y, PL, ramp_slope, H, alpha, binGop, D)
...
% if Pref is less than Pmin for any generator

Pref = zeros(numG,1);
PL_remaining = PL;
for i = 1:numG
    if Pref(i) < Pmin(i) && binGop(i) == 1  
        PL_remaining = PL_remaining - alpha(i)*Pmin(i);
        Pref(i) = Pmin(i);
    end
end
Pref = Pref + ones(numG,1) * (PL_remaining) / sum(alpha);
...
%% Equations
    ...
    dydt = [(1/(2*H)) * (alpha' * Pm - PL_net - D * dw);
