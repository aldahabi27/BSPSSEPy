...
class sim:
    ...
    async def InitializeDynamicSimulation(self, app=None):
        ...
        if self.debug_print:
            bp(
                "[DEBUG] Dynamic simulation parameters set successfully.",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
        ...
        if ierr == 0:
            bp(
                "[SUCCESS] set_genpwr enabled with a threshold of 0.01 MW.",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
        else:
            bp(
                f"[ERROR] Failed to enable set_genpwr. Error code: {ierr}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
    ...
    async def GetAllElements(self, app=None):
        ...
        if self.debug_print:
            bp(
                f"[DEBUG] Retrieved branch info from PSSE:\n{bspssepy_brn}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
        ...
        bspssepy_brn["BSPSSEPyStatus_0"] = bspssepy_brn[
            "BSPSSEPyStatus"
        ]  # Initial status when the simulation is started
        ...
        if self.debug_print:
            bp(
                f"[DEBUG] Retrieved bus info from PSSE:\n{bspssepy_bus}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)

        # Add metadata columns
        bspssepy_bus["BSPSSEPyType_0"] = bspssepy_bus[
            "TYPE"
        ]  # Same info of TYPE
        bspssepy_bus["BSPSSEPyStatus"] = bspssepy_bus["TYPE"].apply(
            lambda x: "Tripped" if x == 4 else "Closed"
        )  # Set BSPSSEPyStatus based on BSPSSEPyType
        bspssepy_bus["BSPSSEPyLastAction"] = "Initialized"
        bspssepy_bus["BSPSSEPyLastActionTime"] = (
            0.0  # Default initialization time
        )
        ...
        bspssepy_load = await get_load_info(
            load_keys=["ID", "LOADNAME", "NUMBER", "NAME", "STATUS"],
            debug_print=self.debug_print,
            app=app,
        )
        ...
        if self.debug_print:
            bp(
                f"[DEBUG] Retrieved load info from PSSE:\n{bspssepy_load}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)

        # Add metadata columns
        bspssepy_load["BSPSSEPyStatus"] = bspssepy_load["STATUS"].apply(
            lambda x: "Enabled" if x == 1 else "Disabled"
        )  # Set BSPSSEPyStatus based on STATUS
        bspssepy_load["BSPSSEPyStatus_0"] = bspssepy_load[
            "BSPSSEPyStatus"
        ]  # Initial status
        bspssepy_load["BSPSSEPyLastAction"] = "Initialized"
        bspssepy_load["BSPSSEPyLastActionTime"] = (
            0.0  # Default initialization time
        )
        ...
        bspssepy_load["BSPSSEPyTiedDeviceType"] = (
            None  # Placeholder for tied device type
        )
        ...
        if self.debug_print:
            bp(
                f"[DEBUG] Retrieved transformer info from PSSE:\n{bspssepy_trn}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)

        # Add metadata columns
        bspssepy_trn["BSPSSEPyStatus"] = bspssepy_trn["STATUS"].apply(
            lambda x: "Closed" if x == 1 else "Tripped"
        )  # Set BSPSSEPyStatus based on STATUS
        bspssepy_trn["BSPSSEPyStatus_0"] = bspssepy_trn[
            "BSPSSEPyStatus"
        ]  # Initial status
        ...
        if ierr == 0:
            ...
            bp(
                f"[ERROR] Error configuring Newton-Raphson method. Error code = {ierr}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
            if app:
        ...
        # ):
        #     bp(bspssepy_gen.to_string(index=False))
        #     await asyncio.sleep(app.async_print_delay if app else 0)

        bspssepy_ibr = bspssepy_gen[
            bspssepy_gen["WMOD"] != 0
        ].copy()  # Filter IBRs (Inverter-Based Resources)

        # Remove any IBR from the bspssepy_gen DataFrame
        ...
        bspssepy_agc = pd.DataFrame(
            {
                ...
                "Δf (Hz)": [0]
                * len(bspssepy_gen),  # Initialize Δf with zeros
                "Δf' (Hz/s)": [0]
        ...
        bp("Creating bspssepy_ibr dataframe", app=app)
        await asyncio.sleep(app.async_print_delay if app else 0)

        # # Create the IBR configuration DataFrame
        # ibr_config_df = pd.DataFrame(self.config.ibr_config)
        ...
        self.bspssepy_ibr = await build_bspssepy_ibr(
            ibr_config=self.config.ibr_config,
            bspssepy_ibr=bspssepy_ibr,
            sim_config=self.config,
            debug_print=self.debug_print,
            app=app,
        )

        # bspssepy_ibr["ibr_type"] = self.config.ibr_config[]
        ...
        #     "display.colheader_justify", "center",  # Center column headers for readability
        # ):
        #     bp(bspssepy_ibr.to_string(index=False))
        #     await asyncio.sleep(app.async_print_delay if app else 0)
        bp(
            "[SUCCESS] All elements retrieved successfully and the data frames are initialized.",
            app=app,
        )
        await asyncio.sleep(app.async_print_delay if app else 0)
    ...
    async def SetBlackStart(self, app=None):
        ...
        if self.debug_print:
            bp(
                "[DEBUG] Configuring system for black-start scenario...",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
        ...
        # ibr_status_df = await GetGenInfo(
        #     ["MCNAME", "NUMBER", "STATUS"],
        #     debug_print=self.debug_print,
        #     app=app,
        # )
                ...
                if self.config.ControlSequenceAsIs:
                    ...
                    if (
                        ...
                        if self.EnforceActionLock and any(
                            ...
                            if self.debug_print:
                                bp(
                                    "[DEBUG] EnforceActionLock active. Skipping new action due to ongoing actions.",
                                    app=app,
                                )
                                await asyncio.sleep(
                                    app.async_print_delay if app else 0
                ...
                if (
                    ...
                    if (
                        ...
                        self.config.bspssepy_sequence.at[
                            action["BSPSSEPySequenceRowIndex"],
                            "Action Status",
                        ] = action["ActionStatus"]

                        if self.debug_print:
                            bp(
                                f"[DEBUG] Updated Action Status for Row {action['BSPSSEPySequenceRowIndex']}: {action['ActionStatus']}",
                                app=app,
                            )
                            await asyncio.sleep(
                                app.async_print_delay if app else 0
                        ...
                        if action["ActionStatus"] == -999:
                            ...
                            if self.debug_print:
                                bp(
                                    f"[CAUTION]{action} - **** This action will be skipped as it should be embedded within GenEnable actions. To avoid this msg, remove the action from the plan! ****",
                                    app=app,
                                )
                                await asyncio.sleep(
                                    app.async_print_delay if app else 0
                        ...
                        self.config.bspssepy_sequence.at[
                            action["BSPSSEPySequenceRowIndex"], "Start Time"
                        ] = action["StartTime"]
                        self.config.bspssepy_sequence.at[
                            action["BSPSSEPySequenceRowIndex"], "End Time"
                        ] = action["EndTime"]
...
async def SetupVoltageFrequencyChannels(config, debug_print=False, app=None):
    ...
    for bus in config.BusesToMonitor_Voltage:
        ...
        if ierr == 0:
            ...
            if debug_print:
                bp(
                    f"[DEBUG] Voltage Angle channel setup for Bus {bus} - Channel Index {config.CurrentChannelIndex}.",
                    app=app,
                )
                await asyncio.sleep(app.async_print_delay if app else 0)
            ...
            bp(
                f"[ERROR] Failed to set up voltage monitoring for Bus {bus}. Error code = {ierr}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
            if app:
                ...
                raise Exception(
                    "Error In SetupVoltageFrequencyChannels function!"
                )
            else:
                sys.exit(1)
    ...
    for bus in config.BusesToMonitor_Frequency:
        ...
        if ierr == 0:
            ...
            bp(
                f"[ERROR] Failed to set up frequency monitoring for Bus {bus}. Error code = {ierr}",
                app=app,
            )
            await asyncio.sleep(app.async_print_delay if app else 0)
            if app:
