...
def generate_generators_data():
    ...
    return [
        [
            "Gen A",
            f"{random.uniform(49.8, 50.2):.2f} Hz",
...
def CreateHeaderSection(SimTime, ProgressValue):
    ...
    RightSection = f"[bold]t = {SimTime}s[/bold]".ljust(
        20
    ) + f"[bold magenta]{ProgressValue:.1f}%[/bold magenta]".rjust(10)

    # Layouts for alignment
    LeftPanel = Panel(
        ProgramInfo,
        title="[bold blue]Simulation Info[/bold blue]",
        border_style="blue",
        expand=True,
    )
    RightPanel = Panel(
        Align.right(RightSection),
    ...
    TablesLayout.split_row(
        Layout(
            Panel(
                ...
                title="[bold red]Control Sequences[/bold red]",
                border_style="red",
            ),
            ratio=1,
        ),
        Layout(
            Panel(
                "AGC Table",
                title="[bold yellow]AGC[/bold yellow]",
                border_style="yellow",
            ),
            ratio=1,
        ),
        Layout(
            Panel(
                ...
                title="[bold cyan]Generator Data[/bold cyan]",
                border_style="cyan",
            ),
            ratio=1,
        ),
    ...
    with Live(refresh_per_second=2) as live:
        ...
        while iterations is None or count < iterations:
            ...
            gen_table = make_table(
                "Generators",
                ["Generator", "Frequency", "Status"],
                generate_generators_data(),
            )
            bus_table = make_table(
                "Buses", ["Bus", "Frequency"], generate_buses_data()
            )
            load_table = make_table(
                "Loads", ["load", "Power"], generate_loads_data()
            )
